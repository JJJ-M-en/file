...
export function SupervisorApp({
  ...
  // ── Edit driver ───────────────────────────────────────────────────────────
  const openEdit = (u) => { setEditModal(u); setEditName(u.name); setEditUser(u.username); setEditPw(""); setEditPlate(u.plate || ""); setEditMsg(""); };
  const doEdit = () => {
    if (!editName.trim()) { setEditMsg("⚠️ El nombre no puede estar vacio"); return; }
    if (!editUser.trim()) { setEditMsg("⚠️ El usuario no puede estar vacio"); return; }
    if (!editPlate.trim()) { setEditMsg("⚠️ La placa no puede estar vacia"); return; }
    ...
    if (editPw.length > 0 && editPw.length < 6) {
      setEditMsg("⚠️ La contrasena debe tener minimo 6 caracteres (o dejala en blanco)"); return;
    }
  ...
  // ── Add driver ────────────────────────────────────────────────────────────
  const openAdd = () => { setAddModal(true); setAddName(""); setAddUser(""); setAddPw(""); setAddPlate(""); setAddMsg(""); };
  const doAdd = () => {
    if (!addName.trim()) { setAddMsg("El nombre no puede estar vacio"); return; }
    if (!addUser.trim()) { setAddMsg("El usuario no puede estar vacio"); return; }
    if (!addPlate.trim()) { setAddMsg("La placa no puede estar vacia"); return; }
    if (addPw.length < 6) { setAddMsg("La contrasena debe tener minimo 6 caracteres"); return; }
    const all = [...users.supervisors, ...users.drivers];
    if (all.find((u) => u.username === addUser.trim())) { setAddMsg("Ese usuario ya existe"); return; }
    const newDriver = {
      id:       "drv" + Date.now(),
      name:     addName.trim(),
      ...
      avatar:   addName.trim().split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase(),
    };
    saveUsers({ supervisors: users.supervisors, drivers: [...users.drivers, newDriver] });
    setAddMsg("Conductor agregado correctamente");
    setTimeout(() => { setAddModal(false); }, 1600);
  };
          ...
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <p style={{ color: C.muted, fontSize: 11, margin: 0 }}>
                Puedes editar o agregar conductores sin limite.
              </p>
              <button onClick={openAdd} style={{ ...css.btn("#22c55e"), fontSize: 12 }}>+ Agregar Conductor</button>
            </div>
            {users.drivers.map((u) => {
      ...
      {addModal && (
        <div style={{
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20,
            ...
            <h3 style={{ color: C.text, margin: "0 0 20px",
              fontFamily: "'Rajdhani',sans-serif", fontSize: 20, letterSpacing: 1 }}>
              ➕ Agregar Conductor
            </h3>
            <label style={{ ...css.label, marginBottom: 14 }}>Nombre completo
              <input style={css.inp} value={addName}
                onChange={(e) => setAddName(e.target.value)} placeholder="Ej. Juan Perez" />
            </label>
            <label style={{ ...css.label, marginBottom: 14 }}>Placa del vehiculo
              <input style={css.inp} value={addPlate}
                onChange={(e) => setAddPlate(e.target.value)} placeholder="Ej. ABC-123" />
            </label>
            <label style={{ ...css.label, marginBottom: 14 }}>Nombre de usuario
              <input style={css.inp} value={addUser}
                onChange={(e) => setAddUser(e.target.value)} placeholder="Ej. jperez" />
            </label>
            <label style={css.label}>Contrasena (min. 6 caracteres)
              <input style={css.inp} type="password" value={addPw}
                onChange={(e) => setAddPw(e.target.value)} placeholder="Contrasena..." />
            </label>
            {addMsg && <p style={{ color: addMsg.includes("correctamente") ? "#22c55e" : "#ef4444",
              fontSize: 12, margin: "10px 0 0" }}>{addMsg}</p>}
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={() => setAddModal(false)} style={{ ...css.btn(C.muted), flex: 1 }}>Cancelar</button>
              <button onClick={doAdd} style={{ ...css.btn("#22c55e"), flex: 2 }}>✅ Agregar</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20,
            ...
            <label style={{ ...css.label, marginBottom: 14 }}>Placa del vehiculo
              <input style={css.inp} value={editPlate}
                onChange={(e) => setEditPlate(e.target.value)} placeholder="Ej. ABC-123" />
            </label>
            <label style={{ ...css.label, marginBottom: 14 }}>Nombre de usuario
              <input style={css.inp} value={editUser}
                onChange={(e) => setEditUser(e.target.value)} placeholder="usuario123" />
            </label>
            <label style={css.label}>Nueva contrasena (opcional, min. 6 caracteres)
